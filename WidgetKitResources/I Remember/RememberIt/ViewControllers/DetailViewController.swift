...
class DetailViewController: UIViewController {
    ...
    var updateItem: ((RememberItItem) -> Void)?
    var newItem:((RememberItItem) -> Void)?
    var deleteItem:((RememberItItem) -> Void)?
    var switchPrimary:((RememberItItem) -> Void)?
    var item:RememberItItem?
    ...
    @objc func addItem() {
        let new = RememberItItem(title: itemTitle.text ?? "", icon: IconString.allCases[iconSelection.selectedSegmentIndex].rawValue , detail1: detail1.text ?? "", detail2: detail2.text ?? "")
        newItem?(new)
        dismissModal()
    }
